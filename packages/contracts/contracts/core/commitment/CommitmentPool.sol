...
abstract contract CommitmentPool is ICommitmentPool, AssetPool, ReentrancyGuard, Sanctions {
  ...
  modifier onlyRollupWhitelisted() {
    if (!rollupWhitelistDisabled && !rollupWhitelist[msg.sender])
      revert CustomErrors.Unexpected("only whitelisted roller.");
    _;
  }

  modifier onlyEnqueueWhitelisted() {
    if (!enqueueWhitelist[msg.sender])
      revert CustomErrors.Unexpected("only whitelisted sender.");
    _;
  }
  ...
  constructor(uint8 _treeHeight) {
    if (_treeHeight <= 0)
      revert CustomErrors.Unexpected("_treeHeight should be greater than 0");
    operator = msg.sender;
    ...
    onlyEnqueueWhitelisted
  {
    ...
    if (_request.rollupFee < minRollupFee)
      revert CustomErrors.RollupFeeToLow("rollup fee too few");
    if (commitmentIncludedCount + commitmentQueueSize >= treeCapacity)
      revert CustomErrors.Unexpected("tree is full");
    if (historicCommitments[_request.commitment])
      revert CustomErrors.Unexpected("the commitment has been submitted");
  ...
  function rollup(RollupRequest memory _request) external override onlyRollupWhitelisted {
    if (_request.newRoot >= DataTypes.FIELD_SIZE)
      revert CustomErrors.Unexpected("newRoot should be less than FIELD_SIZE");
    if (rootHistory[_request.newRoot])
      revert CustomErrors.Duplicated("newRoot is duplicated");
    if (
      _request.rollupSize <= 0 ||
      _request.rollupSize > commitmentQueueSize ||
      !rollupVerifiers[_request.rollupSize].enabled
    )
      revert CustomErrors.Invalid("invalid rollupSize");

    uint256 includedCount = commitmentIncludedCount;
    if (includedCount % _request.rollupSize != 0)
      revert CustomErrors.Invalid("invalid rollupSize at current state");
    ...
    for (uint256 index = 0; index < _request.rollupSize; index++) {
      ...
      if (leaf.commitment == 0)
        revert CustomErrors.Unexpected("index out of bound");
    ...
    if (_request.leafHash != expectedLeafHash)
      revert CustomErrors.Invalid("invalid leafHash");
    ...
    if (!verified)
      revert CustomErrors.Invalid("invalid proof");
  ...
  function transact(TransactRequest memory _request, bytes memory _signature) external override nonReentrant {
    ...
    if (!transactVerifiers[numInputs][numOutputs].enabled)
      revert CustomErrors.Invalid("invalid i/o length");
    if (_request.sigHashes.length != numInputs)
      revert CustomErrors.Invalid("invalid sigHashes length");
    if (_request.outRollupFees.length != numOutputs)
      revert CustomErrors.Invalid("invalid outRollupFees length");
    if (_request.outEncryptedNotes.length != numOutputs)
      revert CustomErrors.Invalid("invalid outEncryptedNotes length");
    ...
    if (_request.sigPk != bytes32(uint256(uint160(recoveredSigPk))))
      revert CustomErrors.Invalid("invalid signature");
    ...
    if (!rootHistory[_request.rootHash])
      revert CustomErrors.Invalid("invalid root");
    ...
    for (uint256 i = 0; i < numInputs; i++) {
      ...
      if (spentSerialNumbers[sn])
        revert CustomErrors.Unexpected("the note has been spent");
      if (sn >= DataTypes.FIELD_SIZE)
        revert CustomErrors.Invalid("the note invalid");
      if (_request.sigHashes[i] >= DataTypes.FIELD_SIZE)
        revert CustomErrors.Invalid("sig hash invalid");
      inputs[i + 1] = sn;
      inputs[i + offsetSigHash] = _request.sigHashes[i];
    }

    if (uint256(_request.sigPk) >= DataTypes.FIELD_SIZE)
      revert CustomErrors.Invalid("invalid sig pk");
    if (_request.publicAmount >= DataTypes.FIELD_SIZE)
      revert CustomErrors.Invalid("invalid amount");
    if (_request.relayerFeeAmount >= DataTypes.FIELD_SIZE)
      revert CustomErrors.Invalid("invalid relayer fee amount");
    ...
    for (uint256 i = 0; i < numOutputs; i++) {
      if (historicCommitments[_request.outCommitments[i]])
        revert CustomErrors.Duplicated("duplicate commitment");
      if (_request.outCommitments[i] >= DataTypes.FIELD_SIZE)
        revert CustomErrors.Invalid("invalid out commitment");
      if (_request.outRollupFees[i] < minRollupFee)
        revert CustomErrors.RollupFeeToLow("rollup fee too low");
      if (_request.outRollupFees[i] >= DataTypes.FIELD_SIZE)
        revert CustomErrors.Invalid("invalid out rollup fee");
    ...
    if (!verified)
      revert CustomErrors.Invalid("invalid transact proof");
  ...
  function enableTransactVerifier(
    ...
    if (verifierUpdateDisabled)
      revert CustomErrors.Unexpected("verifier updates have been disabled.");
    if (_numInputs <= 0)
      revert CustomErrors.Unexpected("numInputs should > 0");
  ...
  function enableRollupVerifier(uint32 _rollupSize, IVerifier _rollupVerifier) external onlyOperator {
    ...
    if (_rollupSize & (_rollupSize - 1) != 0)
      revert CustomErrors.Unexpected("rollup size not power of 2");
  ...
  function setMinRollupFee(uint256 _minRollupFee) external onlyOperator {
    if (_minRollupFee <= 0)
      revert CustomErrors.Invalid("invalid _minRollupFee");
  ...
  function _transactRequestHash(TransactRequest memory _request) internal returns (bytes32) {
    ...
    if (outNotesLen >= 3)
      revert CustomErrors.Unexpected("output notes less than 3");
